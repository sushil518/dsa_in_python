...
class DoubleLinkedList(object):
    ...
    def delete_first_node(self):
        if self.start is None:  # list is empty
            if self.start is None:
                return
        if self.start.next is None: # List has onely one node
            self .start = None
            return
    ...
    def delete_last_node(self):
        if self.start is None:
            return
        if self.start.next is None: # list has only one nodee
            self.start = None
            returnr
        p = self.start
        while p.next != None:
    ...
    def delete_node(self,x):
        ...
        if p.next is not None: # node to be deleted is in between
            p.prev.next = p.next
            p.next.prev = p.prev
        else: # p referes to last node
            if p.info == x: # node to be delelted is last node
                p.prev.next = None
            else:
                print(x, " not found")
...
while True:
    ...
    print("3.Insert a node in the begininning of the list")
    print("4. Insert a node at the end of the list")
    print("5.Insert a node before a specified node")
    print("6.Insert a node bofore a specified node")
